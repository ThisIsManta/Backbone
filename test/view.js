(function() {
	...
	test("in-scope events", 5, function() {
		var count = 0;

		window.isTesting = true;
		var view = new (Backbone.View.extend({
			el: $('body'),
			"click@document body": function() {
				count++;
			},
			"onResize, resize@window": function() {
				count++;
			},
			"change last@custom": function() {
				count *= 2;
			},
			"change@custom": function() {
				count++;
			},
			"change first@custom": function() {
				count *= -1;
			}
		}));
		window.isTesting = undefined;
		...
		count = 0;
		view.on('change', function () { count++; }).trigger('change');
		equal(count, 4);
	});
