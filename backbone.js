...
(function(root, factory) {
	...
	var triggerEvents = function(events, args) {
		...
		switch (args.length) {
			case 0:
				while (++i < l && t !== false) t = (ev = events[i]).callback.call(ev.ctx);
				return;
			case 1:
				while (++i < l && t !== false) t = (ev = events[i]).callback.call(ev.ctx, a1);
				return;
			case 2:
				while (++i < l && t !== false) t = (ev = events[i]).callback.call(ev.ctx, a1, a2);
				return;
			case 3:
				while (++i < l && t !== false) t = (ev = events[i]).callback.call(ev.ctx, a1, a2, a3);
				return;
			default:
				while (++i < l && t !== false) t = (ev = events[i]).callback.apply(ev.ctx, args);
		}
	};
	...
	var View = Backbone.View = function(options) {
		...
		this.id = this.cid = _.uniqueId('view');
		
		// Merge everything from the argument.
		_.extend(this, options);
		
		// Set default values.
		this.isRendered = typeof this.isRendered === 'boolean' ? this.isRendered : false;
		this.isVisible = typeof this.isVisible === 'boolean' ? this.isVisible : true;
						...
						if (eventName.indexOf('@') > 0) {
							...
							if (target === 'window') {
								...
								if (windowEventCore._events === undefined || windowEventCore._events[action] === undefined) {
									broker = _.defer(_.bind(windowEventCore.trigger, windowEventCore, action));
	...
	_.extend(View.prototype, Events, {
		...
		find: function (chid) {
			if (typeof chid === 'string') {
				for (var name in this) {
					if (name !== 'parent' && this[name] instanceof View) {
						if (name === chid) {
							return this[name];
						} else {
							if (typeof this[name].find !== 'function') {
								this[name].find = this.find;
							}
							var temp = this[name].find(chid);
							if (temp !== null) {
								return temp;
							}
						}
		...
		remove: function() {
			...
			// Remove this view itself and unbind managed event(s).
			this.$el.remove();
			
			// Unbind unmanaged event(s) associated with this view.
			_.each(this._unmanagedEventList, function (current) {
				if (current.target === window) {
					$(window).off(current.broker);
					windowEventCore.off(current.action, current.method);
				} else {
					$(current.target).off(current.action, current.method);
				}
			});
